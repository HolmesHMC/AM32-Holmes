...
*1.83  -Add stall protection pid loop.
           ...
           -decrease speed isr_hot.step re-entering sine mode
...
HOT_ISR_FN void zcfoundroutine(void);
extern void transfercomplete(); /* Refactor 3.2: declared in signal.c */
...
uint32_t polling_mode_changeover;    // written once at init, read in main only
uint8_t ramp_divider;                // written once at init before ISR starts
/* Refactor 3.6: Ramp rate lookup table — computed at init, indexed by phase:
 * [0] = startup (low zero_crosses or low duty_cycle)
 * [1] = low RPM  (average_interval > 500)
 * [2] = high RPM (average_interval <= 500)
 * Replaces branching if/else chain with single table lookup.           */
uint8_t ramp_rate_lut[3] = {
    RAMP_SPEED_STARTUP,    /* [0] startup */
    RAMP_SPEED_LOW_RPM,    /* [1] low RPM */
    RAMP_SPEED_HIGH_RPM,   /* [2] high RPM */
};
/* Legacy aliases kept for init code readability */
#define max_ramp_startup  ramp_rate_lut[0]
#define max_ramp_low_rpm  ramp_rate_lut[1]
#define max_ramp_high_rpm ramp_rate_lut[2]
volatile char send_esc_info_flag;    // set in ISR, read in main — needs volatile
uint32_t eeprom_address = EEPROM_START_ADD; 
uint16_t prop_brake_duty_cycle = 0;
uint16_t ledcounter = 0;
uint8_t ramp_count;          /* Max value: ramp_divider (0 or 9), fits in uint8_t */
uint32_t process_time = 0;
uint32_t start_process = 0;
uint8_t one_khz_loop_counter = 0;  /* Max value: PID_LOOP_DIVIDER (20), fits in uint8_t */
uint16_t target_e_com_time_high;
uint16_t target_e_com_time_low;
volatile uint8_t compute_dshot_flag = 0;  /* 5.2: hot-path reads wrapped with #ifndef NO_DSHOT */
...
/* Refactor 3.1: isr_hot.commutation_intervals and isr_hot.average_interval moved into isr_hot struct */
uint32_t e_com_sum = 0;  /* running sum of isr_hot.commutation_intervals[0..5] */
...
/* Refactor 3.5: Pre-scaled sine table — computed once at init in initSineTable().
 * Stores: ((2*pwmSin[i]/SINE_DIVIDER + gate_drive_offset) * TIMER1_MAX_ARR / 2000)
 *         * eepromBuffer.sine_mode_power / 10
 * Eliminates 3 multiply/divide ops per advanceincrement() call.           */
...
/* Refactor 3.1: isr_hot.bad_count moved into isr_hot struct */
uint8_t bad_count_threshold = CPU_FREQUENCY_MHZ / 24;

/* Refactor 3.8: Cached EEPROM fields — loaded once at init, read-only after.
 * Avoids struct-offset dereferences in hot ISR/main-loop paths.       */
uint8_t ec_auto_advance  = 0; /* eepromBuffer.auto_advance  — used in TIMER ISR */
uint8_t ec_bi_direction  = 0; /* eepromBuffer.bi_direction  — used in main loop */
uint8_t ec_variable_pwm  = 0; /* eepromBuffer.variable_pwm  — used in main loop */
uint8_t ec_comp_pwm      = 0; /* eepromBuffer.comp_pwm      — used in commutate */
...
/* Refactor 3.1: ISR hot data struct — 36 bytes, cache-line aligned.
 * Fields: isr_hot.zcfound, isr_hot.bemfcounter, isr_hot.lastzctime, isr_hot.thiszctime, isr_hot.duty_cycle, isr_hot.step,
 * isr_hot.commutation_interval, isr_hot.waitTime, isr_hot.average_interval, isr_hot.advance, isr_hot.rising,
 * isr_hot.bad_count, isr_hot.commutation_intervals[].  Access via isr_hot.fieldname.    */
ISR_HotData_t isr_hot __attribute__((aligned(32))) = {
...
volatile uint8_t transfercomplete_pending = 0; /* Refactor 3.2: set in DMA ISR, processed in main loop */
...
{
    /* Refactor 4.5: Branchless clamping via conditional expressions so the
     * compiler can emit conditional-move instructions instead of branches.
     * Ki/Kp/Kd shift-scaling (>> 8) skipped — requires EEPROM re-tune. */
    pidnow->error = actual - target;
    pidnow->integral = pidnow->integral + pidnow->error * pidnow->Ki;

    /* Branchless integral clamp — avoids branch misprediction cost */
    int32_t int_val = pidnow->integral;
    int32_t ilimit  = pidnow->integral_limit;
    int_val = (int_val >  ilimit) ?  ilimit : int_val;
    int_val = (int_val < -ilimit) ? -ilimit : int_val;
    pidnow->integral = int_val;

    pidnow->derivative = pidnow->Kd * (pidnow->error - pidnow->last_error);
    pidnow->last_error = pidnow->error;

    int32_t pid_out = pidnow->error * pidnow->Kp + int_val + pidnow->derivative;

    /* Branchless output clamp */
    int32_t olimit = pidnow->output_limit;
    pid_out = (pid_out >  olimit) ?  olimit : pid_out;
    pid_out = (pid_out < -olimit) ? -olimit : pid_out;

    pidnow->pid_output = pid_out;
    return pid_out;
}
...
void loadEEpromSettings()
{
    ...
    if (eepromBuffer.advance_level < 43 && eepromBuffer.advance_level > 9 ) { // new format subtract 10 from isr_hot.advance
    ...
    if (eepromBuffer.eeprom_version > 0) { // these commands weren't introduced until eeprom version 1.
        ...
        if (eepromBuffer.input_type < 10) {
            ...
            case AUTO_IN:
                ...
                servoPwm = 0;
#ifndef NO_DSHOT
                EDT_ARMED = 1;           /* 5.2: EDT stub already = 1 */
#endif
                break;
            case DSHOT_IN:
#ifndef NO_DSHOT
...
uint16_t getSmoothedCurrent()
{
    ...
    smoothedcurrent = (total * 1311U) >> 16;  // ~1/50 via multiply-shift (1311/65536 = 1/50.07)
    return smoothedcurrent;
}

/*
 * Refactor 3.4: getBemfState() optimised with lookup table + single comparison.
 *
 * For MCU_F031/G031: phase port/pin arrays indexed by step eliminate the
 * 3-case if-chain (saves 4-6 cycles per BEMF poll in polling mode).
 *
 * For all targets: the double-nested if(rising)/if(current_state) collapses
 * to a single "current_state == rising" XOR comparison — saves 2-4 cycles
 * per call (called hundreds of times per electrical revolution).
 */
#if defined(MCU_F031) || defined(MCU_G031)
/* Phase port/pin lookup tables indexed by step (1-6).  Index 0 unused. */
typedef struct { volatile uint32_t *idr_reg; uint32_t pin; } bemf_phase_t;
static const bemf_phase_t bemf_phase_lut[7] = {
    { 0, 0 },                                              /* [0] unused */
    { &PHASE_C_EXTI_PORT->IDR, PHASE_C_EXTI_PIN }, /* step 1: C floats */
    { &PHASE_A_EXTI_PORT->IDR, PHASE_A_EXTI_PIN }, /* step 2: A floats */
    { &PHASE_B_EXTI_PORT->IDR, PHASE_B_EXTI_PIN }, /* step 3: B floats */
    { &PHASE_C_EXTI_PORT->IDR, PHASE_C_EXTI_PIN }, /* step 4: C floats */
    { &PHASE_A_EXTI_PORT->IDR, PHASE_A_EXTI_PIN }, /* step 5: A floats */
    { &PHASE_B_EXTI_PORT->IDR, PHASE_B_EXTI_PIN }, /* step 6: B floats */
};
#endif
...
#if defined(MCU_F031) || defined(MCU_G031)
    /* Single indexed lookup replaces 3-way if-chain (Refactor 3.4) */
    const bemf_phase_t *ph = &bemf_phase_lut[isr_hot.step];
    current_state = (uint8_t)((*ph->idr_reg) & ph->pin);
#else
    current_state = (uint8_t)(!getCompOutputLevel()); /* polarity reversed */
#endif

    /*
     * Single comparison: bemf expected high when rising, low when falling.
     * current_state == (uint8_t)isr_hot.rising means BEMF polarity matches
     * expectation — count it.  Otherwise it's noise — count as bad.
     * (Refactor 3.4: replaces double-nested if(rising)/if(current_state))
     */
    if ((current_state != 0) == (isr_hot.rising != 0)) {
        isr_hot.bemfcounter++;
    } else {
        isr_hot.bad_count++;
        if (isr_hot.bad_count > bad_count_threshold) {
            isr_hot.bemfcounter = 0;
        }
    }
}

HOT_ISR_FN void commutate()
{
    if (forward == 1) {
        isr_hot.step++;
        if (__builtin_expect(isr_hot.step > 6, 0)) {  /* Rare: only once per electrical revolution */
            isr_hot.step = 1;
            desync_check = 1;
        }
        isr_hot.rising = isr_hot.step & 1;
    } else {
        isr_hot.step--;
        if (__builtin_expect(isr_hot.step < 1, 0)) {  /* Rare: only once per electrical revolution */
            isr_hot.step = 6;
            desync_check = 1;
        }
        isr_hot.rising = !(isr_hot.step & 1);
...
#ifdef INVERTED_EXTI
    isr_hot.rising = !isr_hot.rising;
#endif
    __disable_irq(); // don't let dshot interrupt
    if (!prop_brake_active) {
        comStep(isr_hot.step);
    }
    __enable_irq();
    changeCompInput();
    /* Refactor 4.4: Polling mode re-entry removed.  Once interrupt sync is
     * achieved old_routine stays 0 — the ESC never falls back to polling at
     * low RPM.  The initial startup transition (zcfoundroutine) is unchanged.
     * Saves 100+ cycles/iteration of polling overhead when running. */
    isr_hot.bemfcounter = 0;
    isr_hot.zcfound = 0;
    // Incremental running sum: subtract old, update slot, add new (2.6/2.8)
    e_com_sum -= isr_hot.commutation_intervals[isr_hot.step - 1];
    isr_hot.commutation_intervals[isr_hot.step - 1] = isr_hot.commutation_interval;
    e_com_sum += isr_hot.commutation_interval;
    
#ifdef USE_PULSE_OUT
	if(isr_hot.step == 1 || isr_hot.step == 4  ){
...
HOT_ISR_FN void interruptRoutine()
{
//   if (isr_hot.average_interval > 125) {
//        if ((INTERVAL_TIMER_COUNT < 125) && (isr_hot.duty_cycle < 600) && (zero_crosses < 500)) { // should be impossible, desync?exit anyway
//           return;
//        }
...
//    }
...
#if defined(MCU_F031) || defined(MCU_G031)
            if (((current_GPIO_PORT->IDR & current_GPIO_PIN) == !(isr_hot.rising))) {
#else
            if (getCompOutputLevel() == isr_hot.rising) {
#endif
                return;
            }
        }
    ...
    SET_AND_ENABLE_COM_INT(isr_hot.waitTime+1); // enable COM_TIMER interrupt
    __enable_irq();
}

void startMotor()
{
    if (__builtin_expect(running == 0, 0)) {  /* Rare: motor usually already running */
        commutate();
        isr_hot.commutation_interval = 10000;
...
/* -----------------------------------------------------------------------
 * Refactor 4.2: setInput() input-mode state-machine helpers.
 * Each mode is a static inline — the compiler expands them at the single
 * call site in setInput(), so there is zero call/return overhead and the
 * optimizer sees the full body when inlining.  The big nested-conditional
 * dispatch is replaced with a clean switch-like top-level.
 * ----------------------------------------------------------------------- */

/* RC-car reverse mode via PWM (bidir + rc_car_reverse + !dshot) */
static inline void setInput_rccar_pwm(void)
{
...
/* Bidirectional standard PWM (bidir + !rc_car_reverse + !dshot).
 * Note: newinput may be clamped to 1000 if direction reversal is blocked
 * by motor speed — the map() call below reads the (possibly modified) value. */
static inline void setInput_bidir_pwm(void)
{
    if (newinput > (1000 + (servo_dead_band << 1))) {
        if (forward == eepromBuffer.dir_reversed) {
            if (((isr_hot.commutation_interval > reverse_speed_threshold) &&
                 (isr_hot.duty_cycle < 200)) || stepper_sine) {
                forward = 1 - eepromBuffer.dir_reversed;
                zero_crosses = 0;
                old_routine = 1;
                maskPhaseInterrupts();
                ...
                newinput = 1000;  /* too fast to reverse — clamp to neutral */
            }
        }
        adjusted_input = map(newinput, 1000 + (servo_dead_band << 1), 2000, 47, 2047);
    }
...
#ifndef NO_DSHOT
/* Bidirectional DShot input — rc_car_reverse or standard bidir.
 * Dead when NO_DSHOT is defined (CM_MINI_DUAL_E230 target). */
static inline void setInput_dshot(void)
{
    if (eepromBuffer.rc_car_reverse) {
        /* DShot RC-car reverse: newinput range 0..2047 with 1048 = neutral */
        if (newinput > 1047) {
            if (forward == eepromBuffer.dir_reversed) {
                adjusted_input = 0;
                prop_brake_active = 1;
        ...
        /* DShot standard bidir — momentum guard prevents rapid reversal */
        if (newinput > 1047) {
            if (forward == eepromBuffer.dir_reversed) {
                if (((isr_hot.commutation_interval > reverse_speed_threshold) &&
                     (isr_hot.duty_cycle < 200)) || stepper_sine) {
...
/* Forward-only: pass newinput straight through (non-bidir mode) */
static inline void setInput_forward_only(void)
{
...
void setInput()
{
    /* Refactor 4.2: Clean top-level dispatch — each path is a self-contained
     * static inline function.  Zero call overhead; compiler sees full body. */
    if (eepromBuffer.bi_direction) {
        if (dshot == 0) {
            if (eepromBuffer.rc_car_reverse) {
                setInput_rccar_pwm();
...
if (!stepper_sine && armed) {
        ...
        if (input < 47 + (80 * eepromBuffer.use_sine_start)) {
            ...
            if (!eepromBuffer.comp_pwm) {
                ...
                phase_A_position = ((isr_hot.step - 1) * 60) + enter_sine_angle;
...
HOT_ISR_FN void tenKhzRoutine()
{ // 20khz as of 2.00 to be renamed
    isr_hot.duty_cycle = duty_cycle_setpoint;
    // Cache volatile 'armed' once — saves 4-5 redundant volatile reads per 20kHz call (2.2)
    register char armed_local = armed;
    tenkhzcounter++;
    ledcounter++;
    ramp_count++;
                ...
                if (adjusted_input == 0) {
                    ...
                    if (armed_timeout_count > LOOP_FREQUENCY_HZ) { // one second
                        if (zero_input_count > 30) {
                            armed = 1;
                            armed_local = 1;  // sync local cache with volatile write
#ifdef USE_LED_STRIP
                            //	send_LED_RGB(0,0,0);
                            delayMicros(1000);
    ...
    // ADC/LVC must run in ALL modes including sine/stepper — moved outside stepper_sine guard
    if (one_khz_loop_counter > PID_LOOP_DIVIDER) {
        PROCESS_ADC_FLAG = 1;
        if (stepper_sine) {
            one_khz_loop_counter = 0;  // reset counter in sine mode (six-isr_hot.step resets it below)
        }
    }
	...
	//				send_LED_RGB(255, 0, 0);
            ...
            if (!isr_hot.zcfound) {
                if (isr_hot.rising) {
                    if (isr_hot.bemfcounter > min_bemf_counts_up) {
                        isr_hot.zcfound = 1;
                        zcfoundroutine();
                    }
                } else {
                    if (isr_hot.bemfcounter > min_bemf_counts_down) {
                        isr_hot.zcfound = 1;
                        zcfoundroutine();
                    }
                }
        ...
        if (one_khz_loop_counter > PID_LOOP_DIVIDER) { // 1khz PID loop
            ...
            if (eepromBuffer.stall_protection && running) { // this boosts throttle as the rpm gets lower, for crawlers
                ...
                stall_protection_adjust += (doPidCalculations(&stallPid, isr_hot.commutation_interval,
        ...
        if (ramp_count > ramp_divider) {
          ramp_count = 0;
            ...
            if (isr_hot.average_interval > 200) {
                max_duty_cycle_change = voltage_based_max_change;
            } else {
                max_duty_cycle_change = voltage_based_max_change * 3;
            }
#else
            /* Refactor 3.6: Single table lookup replaces 3-branch if/else
             * idx: 0=startup, 1=low_rpm (interval>500), 2=high_rpm          */
            {
                const int _startup = (zero_crosses < 150) | (last_duty_cycle < 150);
                const int _ramp_idx = _startup ? 0 : (int)(1u + (unsigned)(isr_hot.average_interval <= 500u));
                max_duty_cycle_change = ramp_rate_lut[_ramp_idx];
            }
...
#endif
            if ((isr_hot.duty_cycle - last_duty_cycle) > max_duty_cycle_change) {
                isr_hot.duty_cycle = last_duty_cycle + max_duty_cycle_change;

            }
            if ((last_duty_cycle - isr_hot.duty_cycle) > max_duty_cycle_change) {
                isr_hot.duty_cycle = last_duty_cycle - max_duty_cycle_change;
            }
            }else{
             isr_hot.duty_cycle = last_duty_cycle;
            }

        if ((armed_local && running) && input > 47) {
            if (ec_variable_pwm) { /* Refactor 3.8: cached */ }
            adjusted_duty_cycle = ((isr_hot.duty_cycle * tim1_arr) / 2000) + 1;
            ...
            if (prop_brake_active) {
              ...
              if((eepromBuffer.brake_on_stop == 2) && armed_local){  // require arming for active brake
                comStep(2);
                adjusted_duty_cycle = DEAD_TIME + ((eepromBuffer.active_brake_power * tim1_arr) / 2000)* 10;
            }else{
                adjusted_duty_cycle = ((isr_hot.duty_cycle * tim1_arr) / 2000);
            }
            }
        }
        last_duty_cycle = isr_hot.duty_cycle;
        SET_AUTO_RELOAD_PWM(tim1_arr);
...
{
#ifndef NO_DSHOT
    /* 5.2: compute_dshot_flag checks are dead on NO_DSHOT targets.
     * Wrapped here to avoid two volatile reads per call at 20kHz. */
...
/* Refactor 3.5: Compute pre-scaled sine table after EEPROM settings load.
 * Must be called after loadEEpromSettings() since TIMER1_MAX_ARR and
 * eepromBuffer.sine_mode_power are needed.                               */
void initSineTable(void)
{
    for (int i = 0; i < 360; i++) {
        /* Non-GIMBAL formula: ((2*pwmSin[i]/SINE_DIVIDER + gate_drive_offset)
         * * TIMER1_MAX_ARR / 2000) * eepromBuffer.sine_mode_power / 10  */
        pwmSin_scaled[i] = (int16_t)(
            (((2 * pwmSin[i] / SINE_DIVIDER) + gate_drive_offset)
             * TIMER1_MAX_ARR / 2000)
            * eepromBuffer.sine_mode_power / 10);
    }
}
...
#ifdef GIMBAL_MODE
    /* GIMBAL_MODE: no SINE_DIVIDER, no sine_mode_power — keep as-is */
...
#else
    /* Refactor 3.5: Use pre-scaled table — eliminates 9 multiply/divide ops per call */
    setPWMCompare1(pwmSin_scaled[phase_A_position]);
    setPWMCompare2(pwmSin_scaled[phase_B_position]);
    setPWMCompare3(pwmSin_scaled[phase_C_position]);
#endif
}

HOT_ISR_FN void zcfoundroutine()
{ // only used in polling mode, blocking routine.
    isr_hot.thiszctime = INTERVAL_TIMER_COUNT;
    SET_INTERVAL_TIMER_COUNT(0);
    isr_hot.commutation_interval = (isr_hot.thiszctime + (3 * isr_hot.commutation_interval)) / 4;
    isr_hot.advance = (temp_advance * isr_hot.commutation_interval) >> 6; //   7.5 degree increments
    isr_hot.waitTime = isr_hot.commutation_interval / 2 - isr_hot.advance;
    while ((INTERVAL_TIMER_COUNT) < (isr_hot.waitTime)) {
        if (zero_crosses < 5) {
            break;
        }
    }
#ifdef MCU_GDE23
    TIMER_CAR(COM_TIMER) = isr_hot.waitTime;
#endif
#ifdef STMICRO
    COM_TIMER->ARR = isr_hot.waitTime;
#endif
#ifdef MCU_AT32
		COM_TIMER->pr = isr_hot.waitTime;
#endif
    commutate();
    isr_hot.bemfcounter = 0;
    isr_hot.bad_count = 0;
    ...
    if (eepromBuffer.stall_protection || eepromBuffer.rc_car_reverse) {
        if (zero_crosses >= 20 && isr_hot.commutation_interval <= 2000) {
            old_routine = 0;
            enableCompInterrupts(); // enable interrupt
        }
    } else {
       if (isr_hot.commutation_interval < polling_mode_changeover) {
            old_routine = 0;
            enableCompInterrupts(); // enable interrupt
        }
    }
...
int main(void)
{
    ...
    loadEEpromSettings();
    initSineTable(); /* Refactor 3.5: pre-scale sine table after EEPROM load */
...
#ifdef BRUSHED_MODE
    // bi_direction = 1;
    isr_hot.commutation_interval = 5000;
    eepromBuffer.use_sine_start = 0;
    maskPhaseInterrupts();
  ...
  startup_max_duty_cycle = startup_max_duty_cycle + 400;
    ...
    /* Refactor 3.8: Initialize EEPROM field cache just before entering main loop.
     * All eepromBuffer field modifications must be done before this point.  */
    ec_auto_advance = eepromBuffer.auto_advance;
    ec_bi_direction = eepromBuffer.bi_direction;
    ec_variable_pwm = eepromBuffer.variable_pwm;
    ec_comp_pwm     = eepromBuffer.comp_pwm;

    while (1) {
e_com_time = (e_com_sum + 4) >> 1; // running sum, updated incrementally in zcfoundroutine (2.6/2.8)
#if defined(FIXED_DUTY_MODE) || defined(FIXED_SPEED_MODE)
        setInput();
#endif
...
#endif
#endif
    /* Refactor 3.2: deferred transfercomplete() — process DMA input completion in main loop */
    if (transfercomplete_pending) {
        transfercomplete_pending = 0;
        transfercomplete();
        processDshot();
    }
     ...
     min_bemf_counts_down = TARGET_MIN_BEMF_COUNTS * 2;
	  ...
	  min_bemf_counts_down = TARGET_MIN_BEMF_COUNTS;
       ...
       RELOAD_WATCHDOG_COUNTER();
        ...
        if (ec_variable_pwm == 1) { /* Refactor 3.8: cached variable_pwm */
            tim1_arr = map(isr_hot.commutation_interval, 96, 200, TIMER1_MAX_ARR / 2,
                TIMER1_MAX_ARR);
        }
        if (ec_variable_pwm == 2) {      // uses automatic range   
          if(isr_hot.average_interval < 250 && isr_hot.average_interval > 100){
            tim1_arr = isr_hot.average_interval * (CPU_FREQUENCY_MHZ/9);
          }
          if(isr_hot.average_interval < 100 && isr_hot.average_interval > 0){
            tim1_arr = 100 * (CPU_FREQUENCY_MHZ/9);
         }
          if((isr_hot.average_interval >= 250) || (isr_hot.average_interval == 0)){
              tim1_arr = 250 * (CPU_FREQUENCY_MHZ/9);
          } 
        }
        if (signaltimeout > (LOOP_FREQUENCY_HZ >> 1)) { // half second timeout when armed
            if (armed) {
                // Soft state reset — instant recovery vs 50-100ms system reset downtime (2.9)
                allOff();
                maskPhaseInterrupts();
                armed = 0;
                running = 0;
                input = 0;
                isr_hot.duty_cycle = 0;
                duty_cycle_setpoint = 0;
                last_duty_cycle = 0;
                inputSet = 0;
                zero_input_count = 0;
                zero_crosses = 0;
                old_routine = 1;               // back to polling mode
                isr_hot.commutation_interval = 12500;  // reset to safe startup value
                e_com_sum = 0;
                input_override = 0;
                stall_protection_adjust = 0;
                use_current_limit_adjust = 2000;
                ...
                signaltimeout = 0;             // reset timeout — ESC ready to re-arm
            }
            if (signaltimeout > LOOP_FREQUENCY_HZ << 1) { // 2 second when not armed
                // Soft state reset — no motor running, just clean up input state (2.9)
                allOff();
                maskPhaseInterrupts();
                armed = 0;
                running = 0;
                ...
                zero_crosses = 0;
                old_routine = 1;
                isr_hot.commutation_interval = 12500;
                e_com_sum = 0;
                input_override = 0;
                stall_protection_adjust = 0;
                use_current_limit_adjust = 2000;
...
#endif
        isr_hot.average_interval = (e_com_time * 21845U) >> 16;  // ~1/3 via multiply-shift, saves 22-37 cycles
        if (desync_check && zero_crosses > 10) {
            if ((getAbsDif(last_average_interval, isr_hot.average_interval) > isr_hot.average_interval >> 1) && (isr_hot.average_interval < 2000)) { // throttle resitricted before zc 20.
                zero_crosses = 0;
                desync_happened++;
                if ((!eepromBuffer.bi_direction && (input > 47)) || isr_hot.commutation_interval > 1000) {
                    running = 0;
                }
                old_routine = 1;
                if (zero_crosses > 100) {
                    isr_hot.average_interval = 5000;
                }
        ...
        {
            /* Cache NVIC priority state — NVIC_SetPriority costs 50+ cycles,
             * skip if state unchanged (common case). */
            static uint8_t last_priority_state = 0xFF;
            uint8_t new_priority_state = (uint8_t)(dshot_telemetry &&
                                         (isr_hot.commutation_interval > DSHOT_PRIORITY_THRESHOLD));
            if (__builtin_expect(new_priority_state != last_priority_state, 0)) {
                last_priority_state = new_priority_state;
                if (new_priority_state) {
        ...
        if (stepper_sine == 0) {
            ...
            /*
             * Refactor 3.7: Event-driven filter level update with hysteresis.
             * Only recompute when average_interval changes by >= 10 counts,
             * or commutation_interval crosses a threshold boundary.
             * Eliminates the map() multiply/divide on every main loop iteration
             * (was running at ~20kHz; now triggered only on RPM change).    */
            {
                static uint32_t _last_filter_interval = 0;
                static uint8_t  _last_filter_zc_band  = 255; /* 0=normal, 1=startup, 2=hyperspeed */
                const uint32_t  _FILTER_HYST = 10;
                const uint32_t _cur_ai = isr_hot.average_interval;
                const uint32_t _cur_ci = isr_hot.commutation_interval;
                uint8_t _cur_zc_band = (_cur_ci < 50u) ? 2u :
                                       ((zero_crosses < 100 && _cur_ci > 500u) ? 1u : 0u);
                uint32_t _ai_diff = (_cur_ai > _last_filter_interval)
                                  ? (_cur_ai - _last_filter_interval)
                                  : (_last_filter_interval - _cur_ai);
                if (_ai_diff > _FILTER_HYST || _cur_zc_band != _last_filter_zc_band) {
                    _last_filter_interval = _cur_ai;
                    _last_filter_zc_band  = _cur_zc_band;
                    if (_cur_zc_band == 2u) {
                        filter_level = 2;   /* hyperspeed: minimal filter */
                    } else if (_cur_zc_band == 1u) {
                        filter_level = 12;  /* startup: maximum filter */
                    } else {
                        filter_level = (uint8_t)map(_cur_ai, 100, 500, 3, 12);
                    }
                }
            }

            if (ec_auto_advance) { /* Refactor 3.8: cached eepromBuffer.auto_advance */
              auto_advance_level = map(isr_hot.duty_cycle, 100, 2000, 13, 23);
            }
