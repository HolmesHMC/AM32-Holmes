...
long map(long x, long in_min, long in_max, long out_min, long out_max)
{
    // Handle inverted input range by normalizing: swap both in and out ranges
    if (in_min > in_max) {
        long temp = in_min; in_min = in_max; in_max = temp;
        temp = out_min; out_min = out_max; out_max = temp;
    }
    if (x >= in_max) return out_max;
    if (x <= in_min) return out_min;
    if (out_min == out_max) return out_min;
    // Direct arithmetic mapping - NO RECURSION (saves 150-250 cycles per call)
    return out_min + ((x - in_min) * (out_max - out_min)) / (in_max - in_min);
}

// getAbsDif is now inline in functions.h for always_inline at call sites
